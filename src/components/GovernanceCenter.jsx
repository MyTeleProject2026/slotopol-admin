...
const probes = [
  ['API', '/ping'], ['Service', '/servinfo'], ['Memory', '/memusage'], ['Disk', '/diskusage'],
  ['Games', '/game/algs'], ['Clubs', '/club/list'], ['Treasury', '/admin/allocations'], ['Policies', '/admin/country-game-profiles'],
]
...
export default function GovernanceCenter() {
  ...
  const refresh = useCallback(async () => {
    ...
    await Promise.all(probes.map(async ([key, path]) => {
      ...
      try {
        const response = await api.get(path, { params: path === '/club/list' ? undefined : { _: Date.now() } })
        next[key] = { ok: true, latency: Math.round(performance.now() - started), data: response.data }
      } catch (e) {
        next[key] = { ok: false, latency: Math.round(performance.now() - started), error: e.response?.data?.what || e.response?.data?.error || e.message }
      }
    }))
    setResults(next)
    setUpdated(new Date())
  ...
  return <section className="governance-center">
    ...
    <div className="governance-layout">
      <article className="control-card"><h3>Server health matrix</h3><div className="health-list">{probes.map(([key, path]) => { const result = results[key]; return <div className="health-row" key={key}><div><strong>{key}</strong><small>{path}</small></div><span className={result?.ok ? 'status-ok' : result ? 'status-error' : ''}>{result ? (result.ok ? `ONLINE · ${result.latency} ms` : 'FAILED') : 'CHECKING'}</span></div> })}</div></article>
